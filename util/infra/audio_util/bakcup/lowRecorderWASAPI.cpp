...
namespace audio_util::recorder
{
    ...
    void LowRecorderWASAPI::m_cleanUp()
    {
        // Helper
    }
    ...
    void LowRecorderWASAPI::m_writeWavHeader(std::ofstream &outFile, unsigned int sampleRate, unsigned int channels, unsigned int totalSamples)
    {
        ...
        uint32_t byteRate   = m_wavSampleRate * m_wavChannels * 2;
        uint16_t blockAlign = m_wavChannels * 2;
        ...
        outFile.write(reinterpret_cast<char*>(&m_wavChannels), 2);
        outFile.write(reinterpret_cast<char*>(&m_wavSampleRate), 4);
    ...
    void LowRecorderWASAPI::m_finalizeWavHeader()
    {
        ...
        uint32_t dataSize  = m_totalSamplesWritten * m_wavChannels * 2;
